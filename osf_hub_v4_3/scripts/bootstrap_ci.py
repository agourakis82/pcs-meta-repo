...
def main():
    ...
    for outcome in out_candidates:
        ...
        for t in k_cols:
            rows.append({"phase": "point", "outcome": outcome, "term": t, "estimate": float(mod0.params.get(t, float("nan")))} )
        # Bootstrap CIs
    ...
    (results_dir / "bootstrap_meta.json").write_text(
        json.dumps({"n": N, "seed": seed, "generated_at": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime())}, indent=2),
        encoding="utf-8",
    )
