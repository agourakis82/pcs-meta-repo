    ...
    for j, rec in enumerate(notes, start=1):
        ...
        if rec.get("concept_edges"):
            ...
            blob.append(
                ...
                + "; ".join(
                    [
                        f"{e['source_entity']}|{e['relation']}|{e['target_entity']}"
                        for e in edges
                    ]
                )
            )
...
if __name__ == "__main__":
    ...
    p.add_argument(
        "--chunks", default="ragpp/data/processed/2025_sweep/YEAR-2025_chunks.jsonl"
    )
    p.add_argument(
        "--notes",
        default="ragpp/data/processed/2025_sweep/YEAR-2025_active_reading_notes.jsonl",
    )
    a = p.parse_args()
