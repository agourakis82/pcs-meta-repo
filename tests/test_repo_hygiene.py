...
def test_docs_ccby():
    ...
    assert (
        "cc by 4.0" in txt or "creative commons" in txt
    ), "Conteúdo CC-BY-4.0 não identificado"
...
def test_readme_sections():
    ...
    txt = p.read_text(encoding="utf-8", errors="ignore")
    assert re.search(
        r"^##\s*License\b", txt, flags=re.I | re.M
    ), "Seção 'License' ausente"
    assert re.search(r"^##\s*(Cite this work|How to cite)\b", txt, flags=re.I | re.M), (
        "Seção de citação ausente",
    )
